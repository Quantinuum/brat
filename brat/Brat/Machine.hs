...
getNodeInputs (g, _) name = M.elems (M.fromList [(tgtPort, src) | (src, _, In _ tgtPort) <- wiresTo name g])
...
run g@((nodes, _), cs) fz t@(EvalNode n ins) = case nodes M.! n of
    ...
    (BratNode (Dummy k) _ _) -> run g fz (Finished [DummyV])
...
miniEval gi@((nodes, _), _) env (Ex node 0) =
