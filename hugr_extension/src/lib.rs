...
pub fn closure_custom_type(ins: Vec<TypeRV>, outs: Vec<TypeRV>, extension_ref: &std::sync::Weak<Extension>) -> CustomType {
    CustomType::new(
        CLOSURE_TYPE_ID,
        [
...
pub fn closure_type(ins: Vec<TypeRV>, outs: Vec<TypeRV>, extension_ref: &std::sync::Weak<Extension>) -> Type {
    closure_custom_type(ins, outs, extension_ref).into()
}
...
/*
        ...
        for ctor in all::<BratCtor>() {
            ...
            let args: Vec<TypeArg> = ctor
                ...
                .iter()
                .map(|p| match p {
                    TypeParam::Runtime { .. } => TypeArg::Runtime(Type::UNIT),
