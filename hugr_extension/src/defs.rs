...
impl MakeOpDef for BratOpDef {
    ...
    fn init_signature(&self, extension_ref: &std::sync::Weak<hugr::Extension>) -> SignatureFunc {
        ...
        match self {
            ...
            Partial => SignatureFunc::CustomFunc(Box::new(PartialSigFun(extension_ref))),
...
struct PartialSigFun<'a>(&'a std::sync::Weak<Extension>);
impl<'a> SignatureFromArgs for PartialSigFun<'a> {
    fn compute_signature(&self, arg_values: &[TypeArg]) -> Result<PolyFuncTypeRV, SignatureError> {
        ...
        match arg_values {
            [partial_inputs, other_inputs, outputs] => {
                ...
                let res_func = closure_type(other_inputs.clone(), outputs.clone(), self.0);
                let mut inputs =
                    vec![
                        closure_type([partial_inputs.clone(), other_inputs].concat(), outputs, self.0)
                            .into(),
                    ];
